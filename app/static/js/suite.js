...
function show_suite_win(win_id, fm_id, method){
...
function manage_frame_suite(win_id, fm_id){
    $('#{0}'.lym_format(fm_id)).form('submit',{
        url: "/api/v1/suite/",
        type: "post",
        success:function(data){
...
function manage_suite_table(win_id, fm_id, method){
    var button = {"create": "创建", "edit": "更新", "delete": "删除"};

    if(method == "create"){
        $('#{0}'.lym_format(win_id)).window({"title": "创建套件"});

    }
    else if(method == "edit" || method == "delete"){
        var row = $('#suite_list').datagrid('getSelected');
        if(row){
            $('#{0}'.lym_format(win_id)).window({"title": "管理套件"});
