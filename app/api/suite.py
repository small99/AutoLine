...
from ..models import AutoSuite, AutoProject, User
from .. import db
...
class Suite(Resource):
    ...
    def get(self):
        ...
        pagination = AutoSuite.query.filter_by(project_id=args["project_id"]).order_by(AutoSuite.id.asc()).paginate(
    ...
    def __create(self, args):
        ...
        suite = AutoSuite.query.filter(and_(AutoSuite.name == args["name"],
                                            AutoSuite.project_id == args["project_id"])).first()
        if suite is None:
            try:
                suite = AutoSuite(name=args["name"],
                                  desc=args["desc"],
    ...
    def __edit(self, args):
        result = {"status": "success",
                  "msg": "操作成功"}
        suite = AutoSuite.query.filter_by(id=args["id"]).first()
        if suite is None:
            result["status"] = "fail"
